...
async function loadProjects() {
    try {
        ...
        document.getElementById('projectsList').innerHTML = html || '<p style="color: #505f76;">프로젝트가 없습니다.</p>';
    } catch (error) {
        console.error('프로젝트 로드 오류:', error);
        document.getElementById('projectsList').innerHTML = '<p style="color: #505f76;">프로젝트를 로드할 수 없습니다.</p>';
    }
}
...
async function loadAdminProjects() {
    try {
        ...
        document.getElementById('adminProjectsList').innerHTML = html || '<p style="color: #505f76;">프로젝트가 없습니다.</p>';
    } catch (error) {
        console.error('프로젝트 로드 오류:', error);
    }
}
...
async function handleContactSubmit(e) {
    ...
    try {
        ...
        if (response.ok) {
            document.getElementById('contactMessage').innerHTML = '<div style="padding: 12px; background: rgba(0, 35, 111, 0.1); border-radius: 8px; color: #00236f;">✓ 메시지가 전송되었습니다.</div>';
            setTimeout(() => {
